...
```{r}
#jpeg(filename = "./figure1.jpeg", width = 1200, height = 600, units = "px", 
#    pointsize = 12, quality = 100, bg = "white")

aggregate %>%
  mutate(BlockType = factor(ifelse(BlockType=="Mostly Punishment", 
                                   "Mostly Loss", "Mostly Win"),
                            levels = c("Mostly Win", "Mostly Loss"))) %>%
  ggplot(aes(x = TrialType, y = PSwitch, col = TrialType)) +
  facet_grid( ~ BlockType, labeller = label_both) +
  geom_point(position = position_jitter(width = 0.1, height = 0.05),
             alpha = 0.5) +
  geom_line(aes(group = HCPID), alpha = 0.5, color='darkgray') + 
  scale_color_brewer(palette = "Set2") +
  scale_x_discrete(labels = c("Win", "Loss", "Neutral")) +
  stat_summary(fun.data = "mean_cl_boot", col="black", size=3, alpha=.5) +
  labs(x = "Feedback", y = "Probability of response switching", 
       title = "The probability of response switching") +
  theme_pander(base_size = 30, lp = "none")      
ggsave("figure1.jpeg", units="in", width=15, height=7, dpi=300)
```
...
```{r}
aov(data=noneutral, RT ~ BlockType * TrialType + 
              Error(HCPID/(BlockType * TrialType)))  %>%
  anova_summary() %>%
  xtable() %>%
  kable(digits=4) %>%
...
ggplot(pf_aggregate, 
  ...
  geom_point(position = position_jitter(width = 0.1, height = 0.05),
             alpha = 0.5) +
  geom_line(aes(group = HCPID), alpha = 0.5, color='gray') + 
  scale_color_brewer(palette = "Set2") +
  stat_summary(fun.data = "mean_cl_boot", col="black", size=1, alpha=.7) +
  theme_pander()
...
```{r}
max_ll = read.csv("../actr-models/model_output/max_loglikelihood.csv")
```
