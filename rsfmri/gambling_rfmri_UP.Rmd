...
ggplotly(ggplot(distTibble, aes(x=R)) +
...
ggplotly(ggplot(dependent, aes(x = factor(strategy), fill=factor(strategy))) +
...
The Yp is calculated by averaged across each round
...
load("./__cache__/brainNetworks.RData")
```
...
```{r }
colors <- c(Uncertain = "#E41A1C", `Sensory/somatomotor Hand` = "#FB8072",
            `Cingulo-opercular Task Control` = "#BC80BD", Auditory = "#D9D9D9",
            `Default mode` = "#CCEBC5", `Memory retrieval?` = "#80B1D3", 
            `Ventral attention` = "#FFED6F", Visual = "#FFFFB3", 
            `Fronto-parietal Task Control` = "#377EB8", Salience = "#8DD3C7", 
            Subcortical = "#BEBADA", Cerebellar = "#FCCDE5", 
            `Dorsal attention` = "#FDB462")
```
...
chordDiagram(W_mat, directional = FALSE, transparency = 0.5, self.link = 2, 
             ...
             annotationTrackHeight = mm_h(c(10, 1)),
             annotationTrack = c("grid", "axis"),
...
title("Procedural Network", cex.main = 2)
```
...
p7 <- df.c1 %>%
  ...
  geom_col(aes(fill = NetworkName)) +
   scale_fill_manual(guide = guide_legend(reverse = T), values = df.c1$Color) +
  scale_x_reverse() +
  theme_pander() +
...
p8 <- df.c2 %>%
  ...
  geom_col(aes(fill=NetworkName)) +
  coord_flip() +
  scale_fill_manual(guide = guide_legend(reverse = T), values = df.c2$Color) +
  theme_pander() +
...
png(filename = "./__cache__/figures3.png",  bg = "transparent", 
    width =3000, height = 1500, units = "px", res = 150)
