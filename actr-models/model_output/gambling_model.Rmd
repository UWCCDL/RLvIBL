...
Load cleaned subject data and select one subj data = `r subjID`
...
The plot shows the pattern of 800 simulation runs (Note: only single parameter set was loaded here)
...
```{r}
m2.1000check <- read.csv("./gs_check/MODEL220210426_100307_fnca_1000check.csv") %>%
  mutate(CurrentResponse = case_when(Response=="j" ~ 3,
                                     Response=="f" ~ 2), RT = as.numeric(RT), 
                                     Epoch = as.numeric(Epoch))

m2.1000check.aggregate <- clean_previous_future(count_responses(m2.1000check)) %>%
    filter(!is.na(ResponseSwitch) & !is.na(CurrentResponse)) %>%
    group_by(Epoch, egs, alpha, r, BlockType, TrialType) %>%
...
ggplot(m2.1000check.aggregate, aes(x = TrialType, y = PSwitch.mean, col = alpha)) +
      facet_grid( ~ BlockType) +
      geom_line(aes(group = alpha), alpha=.5) + 
      geom_point(alpha=.5, size=3) +
      geom_errorbar(aes(ymin=PSwitch.mean-PSwitch.sd, ymax=PSwitch.mean+PSwitch.sd), width=.1, position = position_dodge(0.05))+
      #ylim(0,1) +
      theme_pander()

m2.1000check.aggregate
```


### Grid Search

We run the grid search simulation for every single subject 
the parameter grid 

```{r}
m1.gsfiles = list.files(path = ".", 
                        pattern = paste(paste("^MODEL1.*", subjID, sep=""), "*_gs.csv$", sep=""), full.names = T)

m2.gsfiles = list.files(path = "./gs_check", 
                        pattern = paste(paste("^MODEL2.*", subjID, sep=""), "*_gs.csv$", sep=""), full.names = T)

m1.gs <- read.csv(m1.gsfiles) %>% 
  mutate(CurrentResponse = case_when(Response=="j" ~ 3,
                                     Response=="f" ~ 2), RT = as.numeric(RT), 
                                     Epoch = as.numeric(Epoch),
         ans = replace_na(ans, 'nil')) 

m2.gs <- read.csv(m2.gsfiles) %>% 
  mutate(CurrentResponse = case_when(Response=="j" ~ 3,
                                     Response=="f" ~ 2), RT = as.numeric(RT), 
                                     Epoch = as.numeric(Epoch)) 
...
find_best_parameter("model1", m1.gs, subj.cfaggregate %>% filter(HCPID==subjID)) %>% slice_min(RMSE, n = 1)
find_best_parameter("model2", m2.gs, subj.cfaggregate %>% filter(HCPID==subjID)) %>% slice_min(RMSE, n = 1)

m1.gs
subj.cfaggregate %>% filter(HCPID==subjID) %>%
  pivot_wider(id_cols = "HCPID", names_from = "")
```
...
The following analysis is to calculate the Log-Likelihood of P(m|x) given human data; the RMSE of model data and human data;
The BIC scores calculated from both RSS and LL.
