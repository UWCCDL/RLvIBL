...
m.PSwitch.aggplot <- function(m.dat, byParam) {
  ...
  if (byParam=="Epoch") {
    res <- ggplot(model1.aggregate, aes(x = TrialType, y = PSwitch, col = Epoch)) +
      facet_grid( ~ BlockType) +
...
m.RT.aggplot <- function(m.dat, byParam) {
  ...
  if (byParam=="Epoch") {
    res <- ggplot(model1.aggregate, aes(x = TrialType, y = RT, col = Epoch)) +
      facet_grid( ~ BlockType) +
      geom_line(aes(group = Epoch), alpha=.5) + 
      geom_point(alpha=.5, size=3) +
      ylim(0,3500) +
...
model1.aggregate <- aggregate.ByParam.CurrentResponse.BlockType(model1.count)
model2.aggregate <- aggregate.ByParam.CurrentResponse.BlockType(model2.count)
```
...
```{r}
subj.PSwitch.aggplot(gambling.cfaggregate, subjID) + ggtitle(paste("SUB", subjID, sep = "-"))
subj.PSwitch.pfaggplot(gambling.pfaggregate, subjID) + ggtitle(paste("SUB", subjID, sep = "-"))
