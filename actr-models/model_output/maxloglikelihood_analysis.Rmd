...
d %>% select(HCPID, best_model) %>% unique() %>% count(best_model) %>%
  mutate(best_model = ifelse(best_model == "m1", "Declarative", "Procedural")) %>%
  ggbarplot(x = "best_model", y="n", label = T, lab.pos = "in", lab.size = 10, lab.col = "white", 
...
```{r}
d %>% select(HCPID, best_model, trial_type, block_type, Pswitch) %>%
  mutate(TrialType = factor(ifelse(trial_type=="Reward", "Win", trial_type), levels = c("Win", "Loss")),
         `Block Type` = factor(ifelse(block_type=="Reward", "Mostly Win", "Mostly Losses"), levels = c("Mostly Win", "Mostly Losses")),
         Group = ifelse(best_model == "m1", "Declarative", "Procedural")) %>%
  ggplot(aes(x = TrialType, y = Pswitch, group=Group, color=Group)) +
...
ggsave("./maxloglikelihood_analysis_files/figure-html/figure1B.jpeg", units="in", width=15, height=5, dpi=300)
```
